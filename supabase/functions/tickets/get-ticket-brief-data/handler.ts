...
export const handler = async (req: Request) => {
  ...
  try {
    const { fileText, supabaseStorageUrl } = await req.json();
    ...
    const scanPdfResponse = await PdfCoService.scanBarcodeOrQRFromPdf(
      supabaseStorageUrl
    );

    const responseData = {
      isRequestSuccessfull: true,
      data: { ...response, qrCodeUri: scanPdfResponse.value },
      error: null,
    };
